...
export default function App() {
  ...
  const selectStudent = (id) => { setFolderId(null); classroom.setStudentId(id); };
  ...
  return <main className="app-shell">
    ...
    <section className="page-content">
      <section className="class-panel" aria-label="Class and student selector">
        ...
        <label className="class-select">{language === 'en' ? 'Class' : '班级'}<select value={classroom.classId ?? ''} onChange={(event) => selectClass(event.target.value)}>{classroom.classes.map((item) => <option key={item.id} value={item.id}>{item.name ?? item.id}</option>)}</select></label>
        <p>{language === 'en' ? 'Current class' : '当前班级'}：{classroom.classes.find((item) => item.id === classroom.classId)?.name ?? '—'}</p>
        <div className="student-tabs">{classroom.students.map((item) => <button key={item.studentId} type="button" className={`student ${classroom.studentId === item.studentId ? 'active' : ''}`} onClick={() => selectStudent(item.studentId)}>♟ {item.studentId === 'me' ? (language === 'en' ? 'Me (my projects)' : '我（我的项目）') : (item.name ?? item.username ?? item.studentId)}</button>)}</div>
      </section>
